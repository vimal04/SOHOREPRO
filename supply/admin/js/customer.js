...
$(document).ready(function()
{
    ...
    $(".manager_ap").click(function()
    {
        var check_val   = $('#manager_ap').attr('checked') ? 1 : 0;
        var check_ok_1  = confirm($('#manager_ap').attr('checked') ? "Are you sure?" : "Are you sure?");
        var comp_id = $(".copm_id").attr("id");
        var cust_id = $(".customer_id").attr("id");
        //$('.cus_adm_succ').html('');
        //$('.cus_adm_succ').show();
        if (check_ok_1 == true){
            $.ajax
                    ({
                        type: "POST",
                        url: "customers_account_ap_type.php",
                        data: "comp_id=" + comp_id + "&cust_id=" + cust_id + "&account_ap_type=" + check_val,
                        beforeSend: loadStart,
                        complete: loadStop,
                        success: function(option)
                        {
