...
if ($_POST['update_product_customer'] == "1") {
    ...
    function DeleteSpecialtyDtls($comp_id, $product_id){
        $query_fav = "DELETE FROM  sohorepro_favorites WHERE comp_id NOT IN(".$comp_id.") AND product_id = '" . $product_id . "'";
        mysql_query($query_fav); 
    }
    ...
    foreach ($all_customer_output as $customers_output){
        ...
        $comp_id_test[]   = compName($sanitize_value);
    }
    $comp_id_test_all   =   implode(",", $comp_id_test); 
    $delete_without_list    =   DeleteSpecialtyDtls($comp_id_test_all, $product_id);         
