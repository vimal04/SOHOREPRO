...
<!--                <div id="edit_address_<?php echo $current_opt['options']; ?>" style="width:98%;float: left;text-align: right;display: none;">
            ...
            <div id="jumbalakka_nmj" style="float: left;width: 100%;margin-top: 5px;">   
...
//                        echo '</pre>';
    ...
    $check_if_exist =   CheckIfExist($user_session_comp, $user_session, $option_id);
...
//    $message .= '</style>';
    ...
    $message .= '<div style="float: left;width: 100%;">';
    $message .= '<div style="float: left;width: 100%;margin-top: 7px;font-weight: bold;margin-bottom: 20px;">DELIVERY METHOD: WILL PICKUP FROM SOHO REPRO</div>';
    $message .= '</div>';  
    }else{
    $message .= '<div style="float: left;width: 100%;">';
    ...
    foreach ($enteredPlot as $entered) {
        ...
        $shipp_add_p = AddressBookPickupSohoCap($entered_needed_sets_final[0]['shipp_id']);
        $message .= '<div style="float: left;">'.$shipp_add_p[0]['address'].'</div>';   
        }                 
            $message .= '</div>';
            $message .= '<div style="float: left;width: 65%;margin-top: 7px;font-weight: bold;">PACKING LIST: </div>';
...
<?php
    ...
    $current_address            =   SelectIdAddressService($edit_address_id);
    
    if($current_address[0]['company_name'] != $_POST['company_name']){
    ...
    $return_address = $return_address_1.$return_address_2.$return_address_3.$select_address[0]['city'].'<br>'.StateName($select_address[0]['state']).'&nbsp;,'.$select_address[0]['zip'];
    if($sql_result){
