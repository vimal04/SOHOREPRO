...
//        $message .= '</div>';
    ...
    if ($original['ftp_link'] != "0" ) {
                ...
                $user_name  = ($original['user_name'] != '0') ? $original['user_name'] : '';
                $password   = ($original['password'] != '0') ? $original['password'] : '';
        $message .= '<div style="float:left;width: 95%;font-weight: bold;color: #000;margin-top: 7px;"> OPTION'.$original['options'].'</div>';
        $message .= '<div style="float: left;width: 50%;margin-top: 5px;font-weight:bold;background-color: #F99B3E;color: #5C5C5C;text-align: center;border: 1px solid #f99b3e;">Provide Link to File</div>';
        $message .= '<div style="float: left;width: 50%;border: 1px solid #F99B3E;/*margin-left: 30px;*/">';        
        $message .= '<div style="float: left;width: 100%;margin-top: 5px;">FTP Link : ' . $link . '</div>';
        $message .= '<div style="float: left;width: 100%;margin-top: 5px;">User Name : ' . $user_name . '</div>';
        $message .= '<div style="float: left;width: 100%;margin-top: 5px;">Password : ' . $password . '</div>';
        $message .= '</div>';
    }
    }
     ...
     foreach ($cust_original_order as $original){   
                    if($original['upload_file'] != ''){
                    $message .= '<div style="float:left;width: 95%;font-weight: bold;color: #000;margin-top: 7px;">OPTION ' .$original['options'].'</div>';
                    $message .= '<div style="width:90%;float: left;padding: 5px;border: 0px solid #F99B3E;">';
                    $message .= '<div style="width: 45%;float: left;border: 1px solid #F99B3E;">';  
                    $message .= '<div style="padding-top: 3px;font-weight: bold;width: 100%;float: left;background-color: #F99B3E;color: #5C5C5C;text-align: center;">File Options</div>';
                    $message .= '<div style="float: left;width: 65%;margin-left: 10px;margin-top: 7px;text-decoration: underline;">Upload File: </div>';                                 
                    $message .= '<div style="float: left;width: 65%;margin-left: 10px;margin-top: 5px;"><a href="http://cipldev.com/supply-new.sohorepro.com/uploads/'.$original['upload_file'].'" target="_blank">'.$original['upload_file'].'</a></div>';
                    $message .= '</div>';
                    $message .= '</div>';
                    }
                }
    ...
    $message .= '<div class="ribbon" id="ribbon_final" style="font-weight: bold;"><span>RECIPIENT 1</span></div>';
    ...
    $message .= '<div style="width: 100%;float: left;margin-top: 7px;"><span style="font-weight: bold;">Send Via: </span></div>';
    ...
    $ship_type_1 . $ship_type_2 . $ship_type_3 . ',&nbsp;' . $delivery_type . ',&nbsp;Account # ' . $entered_needed_sets_final[0]['billing_number'];
    $message .= '</div>';
    } else {                        
    $message .= '<div style="width: 100%;float: left;margin-top: 7px;">';
    $message .= '<span style="font-weight: bold;">Send Via: </span>';
    $message .= '</div>';
    $message .= '<div style="width: 100%;float: left;margin-top: 7px;">SOHO TO ARRANGE DELIVERY</div>';
    }
    if($entered_needed_sets_final[0]['spl_inc'] != ''){
    $message .= '<div style="width: 100%;float: left;margin-top: 7px;">';    
    $message .= '<span style="font-weight: bold;">Special Instructions: </span>';
    $message .= '</div>';
    ...
    $message .= '<div style="float: left;" class="shaddows">';
    $message .= '<div class="ribbon" style="font-weight: bold;" id="ribbon_final"><span>RECIPIENT 1</span></div>';
    $message .= '<div style="width: 100%;float: left;margin-top: 10px;margin-bottom: 10px;">';
    $message .= '<div class="details_div">';               
    $message .= '<div style="float: left;width: 65%;margin-top: 7px;font-weight: bold;">Send to: </div>';
    ...
    $message .= '<div style="float: left;">'.$shipp_add[0]['company_name'].$att.$phone.'<br>'. $shipp_add[0]['address_1'] . '<br>' . $add_2 . $shipp_add[0]['city'] . '&nbsp;' . StateName($shipp_add[0]['state']) . '&nbsp;' . $shipp_add[0]['zip'].'<br>'.$shipp_add[0]['phone'].'</div>';
    ...
    foreach ($enteredPlot as $entered) {
        ...
        $message .= '<div style="float: left;width: 65%;margin-top: 7px;">';
        $date_asap  = ($entered_needed_sets_final[0]['shipp_time'] != 'ASAP') ? '&nbsp;&nbsp;&nbsp;' . $entered_needed_sets_final[0]['shipp_time'] : '';
        $message .= '<span style="font-weight: bold;">When Needed: </span>'.$entered_needed_sets[0]['shipp_date'].$date_asap.'</div>';
        if ($entered_needed_sets_final[0]['delivery_type'] != '0') {
        $message .= '<div style="width: 100%;float: left;margin-top: 7px;"><span style="font-weight: bold;">Send Via: </span></div>';
        $message .= '<div style="width: 100%;float: left;margin-top: 7px;">';
            if ($entered_needed_sets_final[0]['delivery_type'] == '1') {
        ...
        if($entered_needed_sets_final[0]['spl_inc'] != ''){
        $message .= '<div style="width: 100%;float: left;margin-top: 7px;"><span style="font-weight: bold;">Special Instructions: </span></div>';
        $message .= '<div style="width: 100%;float: left;margin-top: 7px;">'.$entered_needed_sets_final[0]['spl_inc'].'</div></div></div></div>';
        }
    }elseif($entered_needed_sets_final[0]['delivery_type_option'] == '3'){
        ...
        $message .= '<div style="float: left;width: 65%;">';        
        $message .= '<div style="float: left;">'. $service_billing_address[0]['comp_name'] . '<br>' . $service_address_1 . $service_address_2 . $service_address_3 . $service_billing_address[0]['comp_city'] . ',&nbsp;' . $service_billing_address[0]['comp_state'] . '&nbsp;' . $service_billing_address[0]['comp_zipcode'] .'</div>';
    ...
    foreach ($entered_needed_sets_final as $entered_sets) {
        ...
        if (($entered_sets['shipp_id'] != 'P1') && ($entered_sets['shipp_id'] != 'P2')) {
            $add_2 = ($shipp_add[0]['address_2'] == '') ? '' : $shipp_add[0]['address_2'] . ',<br>';
            $message .= $shipp_add[0]['company_name'] . '<br>' . 'Attention: ' . $entered_sets['attention_to'] . '<br>' . 'Tel:  ' . $entered_sets['contact_ph'] . '<br>' . $shipp_add[0]['address_1'] . ',<br>' . $add_2 . $shipp_add[0]['city'] . ',&nbsp;' . StateName($shipp_add[0]['state']) . '&nbsp;' . $shipp_add[0]['zip'];
        } else {                    //echo $shipp_add[0]['address'];                        
            $shipp_add_p = AddressBookPickupSohoCap($entered_sets['shipp_id']);
...
<?php
    ...
    $return_address_1   = ($select_address[0]['address_1'] != '') ? $select_address[0]['address_1'].'<br>' : '';
    $return_address_2   = ($select_address[0]['address_2'] != '') ? $select_address[0]['address_2'].'<br>' : '';
    $return_address_3   = ($select_address[0]['address_3'] != '') ? $select_address[0]['address_3'].'<br>' : '';
    ...
    if($sql_result){
        echo '1~'.$return_address.'~'.$select_address[0]['attention_to'].'~';
