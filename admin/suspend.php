...
{ 
  ...
  $image_change      = (CheckStatusEmailSettings($status_change_id) == '1') ? 'images/active.png' : 'images/de-active.png'; 
    if ($sql_result) {
        echo $image_change.'~'.$comp_id_status;
    }  
}
...
{ 
  ...
  $rec_id               = $_POST['rec_id'];  
  $comp_id_status       = (CheckStatusEmailSettingsService($rec_id, $service_id) == '1') ? '0' : '1';  
  $sql = "UPDATE sohorepro_email SET $service_id = '" . $comp_id_status . "' WHERE id = '" . $rec_id . "'";  
  $sql_result = mysql_query($sql);
    if ($sql_result) {
        echo $comp_id_status;
    }  
