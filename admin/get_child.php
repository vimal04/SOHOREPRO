...
if (isset($_POST['account_type_customer']) && $_POST['account_type_customer'] != '') {   
    $account_type_customer = mysql_real_escape_string($_POST['account_type_customer']);
    $comp_id = mysql_real_escape_string($_POST['comp_id']);
    $invoice_change = "UPDATE sohorepro_company SET cus_type = '" . $account_type_customer . "' WHERE comp_id = '" . $comp_id . "' ";
    mysql_query($invoice_change);    
    if($account_type_customer == '1'){
        echo 'ACCOUNT';
    }elseif($account_type_customer == '2'){
        echo 'CASH';
    }elseif ($account_type_customer == '3') {
        echo 'CASH-EXEMPT';
    }
}

if (isset($_POST['admin_users']) && $_POST['admin_users'] != '') {   
    
    $user_name_set          = mysql_real_escape_string($_POST['user_name_set']);
    $mail_id_set            = mysql_real_escape_string($_POST['mail_id_set']);
    $type_set               = mysql_real_escape_string($_POST['type_set']);
    $password_set           = mysql_real_escape_string($_POST['password_set']);
    $ini_set                = mysql_real_escape_string($_POST['ini_set']);
    $status_set             = mysql_real_escape_string($_POST['status_set']);  
    
    if($type_set == '2'){
        $sql = "INSERT INTO sohorepro_users
			SET     user_name   = '" . $mail_id_set . "',
                                password    = '" . $password_set . "',
                                initials    = '" . strtoupper($ini_set)."',
				email       = '" . $mail_id_set . "',
                                type        = '" . $type_set . "',
                                status      = '1' ";
        $sql_result_staff = mysql_query($sql);
        if($sql_result_staff){
            echo '0';
        }
    }else{
        $sql = "INSERT INTO sohorepro_email
                            SET     name = '" . $user_name_set . "',
                                    email_id = '" . $mail_id_set . "',
                                    status = '" . $status_set . "' ";
        $sql_result_admin = mysql_query($sql);
        if($sql_result_admin){
            echo '1';
        }
    }
...
if (isset($_POST['get_all_customers']) && $_POST['get_all_customers'] != '') {   
    
    $supercategory_name    =   $_POST['supercategory_name_spl'];
    $category_name         =   $_POST['category_name_spl'];
    $subcategory_name      =   $_POST['subcategory_name_spl'];
    $product_name          =   $_POST['product_name_spl'];
    $list_price            =   $_POST['list_price_spl'];
    $discount              =   $_POST['discount_spl'];
    $sell_price            =   $_POST['sell_price_spl'];
    
    
    if($_POST['speciality_on_off_val_spl'] == '1'){
        ...
        $all_customer = $_POST['all_customer_spl'];        
...
//        echo '<pre>';
//        print_r($replace_final_customer_array);
//        echo '</pre>';
//        exit;
        
        // Clean the customers Name and Customer ID End
    ...
    $last_insert_product_id = mysql_insert_id();
    
    $replace_final_customer_array_pre   = str_replace('&amp;', '&', $replace_final_customer_array);
//    
//    echo '<pre>';
//    print_r($replace_final_customer_array_pre);
//    echo '</pre>';
//    exit;
    
               
    foreach ($replace_final_customer_array_pre as $customer)
        {
        $comp_id   = compName(mysql_real_escape_string($customer));          
//        echo 'Company ID:'.$comp_id.'Company Name:'.$customer;
//        exit;
        ...
        $fav_added = mysql_query($query_fav);    
        }
    if($fav_added){
        echo '1';
    }
...
if (isset($_POST['std_spl']) && $_POST['std_spl'] != '') { 
    
    $fav_product_id     =   $_POST['fav_product_id'];
    $spl_text           =   ($_POST['spl_text'] == '1') ? '0' : '1';
    
    $speciality_status  =   CheckSpecialityStatus($fav_product_id);
    
    $speciality_status  =   ($speciality_status[0]['speciality'] == '1') ? '0' : '1';
    
    $sql = "UPDATE sohorepro_products
			SET     speciality      = '".$speciality_status."' WHERE id = '".$fav_product_id."'  ";
    $sql_result = mysql_query($sql);     
    if($speciality_status == '1'){
        echo '1~SPL~spl_std';
    }  else {                
        echo '9~STD~std_spl';
    }
...
if (isset($_POST['delete_order_item']) && $_POST['delete_order_item'] != '') { 
    
    $item_id            =   $_POST['item_id'];
    $order_id           =   $_POST['order_id'];    
    $sql = "DELETE FROM sohorepro_product_master WHERE id = " . $item_id . " ";
    $sql_result = mysql_query($sql);   
    if($sql_result){
        echo '1';
    }  else {                
