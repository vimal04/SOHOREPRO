...
<title>Favorites Products For - <?php echo $company_name; ?></title>
...
<tbody>
    ...
    if (count($favorites) > 0) {
        ...
        foreach ($favorites as $Special_Product) {
            ...
            $product_name = getorderProd($Special_Product['product_id']);
            $list_price = getListPrice($Special_Product['product_id']);
            $discount_price = getDiscountPrice($Special_Product['product_id']);
            $selling_price = getSellPrice($Special_Product['product_id']);
            ?> 
